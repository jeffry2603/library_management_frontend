...
const Header = () => {
  ...
  return (
    <>
      <header className="absolute top-0 bg-[#78909C] w-full py-4 px-6 left-0 shadow-md flex justify-between items-center">
        {/* LEFT SIDE */}
        <div className="flex items-center gap-2">
          <img src={userIcon} alt="userIcon" className="w-8 h-8" />
          <div className="flex flex-col">
            <span className="text-sm font-medium sm:text-lg lg:text-xl sm:font-semibold text-white">
              {user && user.name}
            </span>
            <span className="text-sm font-medium sm:text-lg sm:font-medium text-white">
              {user && user.role}
            </span>
          </div>
        </div>
        {/* RIGHT SIDE */}
        <div className="hidden md:flex items-center gap-2">
          <div className="flex flex-col text-sm lg:text-base items-end font-semibold text-white">
            <span>{currentTime}</span>
            <span>{currentDate}</span>
          </div>
          <span className="bg-white h-14 w-[2px]" />
          <img
            src={settingIcon}
            alt="settingIcon"
